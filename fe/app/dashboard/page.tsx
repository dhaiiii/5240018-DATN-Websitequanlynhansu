...
export default function DashboardPage() {
    ...
    const fetchDashboardData = async () => {
        ...
        const timekeepingRes = await responses[responses.length - 1].json();
...
const handleSetCompanyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            message.success({ content: `Đã lưu tọa độ công ty mới! (Lat: ${updatedLoc.latitude.toFixed(4)})`, key: 'gps' });
            setIsSettingLocation(false);
        },
        (error) => {
